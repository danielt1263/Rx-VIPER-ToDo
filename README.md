...
Again we are struck with the difference in complexity between MVVM-C and VIPER; in the former there would only be three different representations of a todo rather than four. Maybe in a larger project it would make more sense; presumably we would find re-use opportunities that wouldn't otherwise exist? In MVVM-C I'm inclined to create such abstractions when they are discovered rather than assuming it at the outset.
